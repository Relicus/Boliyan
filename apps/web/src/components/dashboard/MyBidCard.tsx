...
export default function MyBidCard({ item, userBid, seller }: MyBidCardProps) {
  ...
  return (
    <>
    <div 
      id={`my-bid-card-${item.slug || item.id}`} 
      onClick={() => setIsModalOpen(true)}
      className="@container p-4 bg-white border rounded-xl flex gap-4 transition-all hover:shadow-sm cursor-pointer"
    >
      <div className="relative shrink-0">
        <img id={`my-bid-img-${item.slug || item.id}`} src={item.images[0]} alt="" className="h-20 w-20 rounded-lg object-cover bg-slate-100" />
      ...
      <div id={`my-bid-content-${item.slug || item.id}`} className="flex-1 min-w-0 flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start">
            <h3 id={`my-bid-title-${item.slug || item.id}`} className="font-bold text-slate-900 truncate mr-2 text-[clamp(1rem,5cqi,1.25rem)]">{item.title}</h3>
            <span id={`my-bid-amount-${item.slug || item.id}`} className="text-[clamp(0.875rem,4cqi,1.125rem)] font-black text-blue-600 shrink-0 font-outfit">
