...
export const DihariLogomark = ({ isActive }: LogoProps) => (
  <svg viewBox="0 0 40 40" className="h-10 w-10 shrink-0 transition-all duration-300 group-hover:drop-shadow-md">
    ...
    <path
      ...
      fill="none"
      strokeLinecap="round"
      className={`transition-all duration-300 ${isActive ? 'text-amber-500' : 'text-slate-800 group-hover:text-amber-500'}`}
    />
    ...
    <g transform="translate(14, 2) scale(0.22)">
      <path
        ...
        className={`transition-all duration-300 ${isActive ? 'fill-amber-500' : 'fill-slate-800 group-hover:fill-amber-500'}`}
      />
    </g>
  </svg>
);

export const DihariWordmark = ({ isActive }: LogoProps) => (
  <div className="flex flex-col items-center justify-center gap-0 py-0.5">
    <span className={`text-2xl font-black mb-[-2px] transition-all duration-300 font-[family-name:var(--font-noto-urdu)] bg-clip-text text-transparent bg-gradient-to-br ${
      isActive 
        ? 'from-amber-600 via-amber-500 to-orange-600' 
        : 'from-slate-950 via-slate-800 to-slate-900 group-hover:from-amber-600 group-hover:via-amber-500 group-hover:to-orange-600'
    }`}>
      دہاڑی
    </span>
    <span className={`text-[10px] font-bold tracking-[0.4em] uppercase transition-all duration-300 font-[family-name:var(--font-outfit)] bg-clip-text text-transparent bg-gradient-to-br ${
      isActive
        ? 'from-amber-400 to-amber-200'
        : 'from-slate-600 to-slate-400 group-hover:from-amber-400 group-hover:to-amber-200'
    }`}>
      Dihari
    </span>
  </div>
