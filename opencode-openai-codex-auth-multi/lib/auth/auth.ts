...
export async function exchangeAuthorizationCode(
	...
	if (!res.ok) {
		...
		console.error(`[${PLUGIN_NAME}] code->token failed:`, res.status, text);
		return { type: "failed" };
	}
	...
	if (
		...
		console.error(`[${PLUGIN_NAME}] token response missing fields:`, json);
		return { type: "failed" };
	}
	return {
		type: "success",
...
export async function refreshAccessToken(refreshToken: string): Promise<TokenResult> {
	try {
		...
		if (!response.ok) {
			...
			console.error(
				`[${PLUGIN_NAME}] Token refresh failed:`,
				response.status,
				text,
			);
			return { type: "failed" };
		...
		if (
			...
			console.error(
				`[${PLUGIN_NAME}] Token refresh response missing fields:`,
				json,
			);
			return { type: "failed" };
		}
		...
		console.error(`[${PLUGIN_NAME}] Token refresh error:`, err);
		return { type: "failed" };
	}
}
