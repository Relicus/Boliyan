...
export async function getCodexInstructions(
	...
	try {
		...
		console.error(
			`[${PLUGIN_NAME}] Failed to fetch ${modelFamily} instructions from GitHub:`,
			err.message,
		);
		...
		if (existsSync(cacheFile)) {
			console.error(
				`[${PLUGIN_NAME}] Using cached ${modelFamily} instructions`,
			);
			return readFileSync(cacheFile, "utf8");
		}
		...
		console.error(
			`[${PLUGIN_NAME}] Falling back to bundled instructions for ${modelFamily}`,
		);
