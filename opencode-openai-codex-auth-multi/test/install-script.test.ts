...
const SCRIPT_PATH = resolve(process.cwd(), 'scripts', 'install-opencode-codex-auth-multi.js');
...
describe('Install script', () => {
	it('updates existing JSONC and preserves comments', () => {
		...
		const configPath = writeConfig(
			...
			`{
  // My existing config
  "plugin": ["some-other-plugin@1.2.3", "opencode-openai-codex-auth-multi@0.1.0"],
  "provider": {
    "openai": {
      "timeout": 60000,
      "models": { "custom-model": { "name": "Custom" } }
		...
		expect(data.plugin).toContain('opencode-openai-codex-auth-multi');
	...
	it('prefers JSONC when both jsonc and json exist', () => {
		...
		const jsoncPath = writeConfig(
			homeDir,
			'opencode.jsonc',
		`{ "plugin": ["opencode-openai-codex-auth-multi@0.1.0"] }`,
		);
  ...
  "provider": {
	...
	it('uninstall --all removes plugin artifacts', () => {
		...
		mkdirSync(join(opencodeDir, 'logs', 'codex-plugin-multi'), { recursive: true });
		mkdirSync(join(opencodeDir, 'cache'), { recursive: true });
		writeFileSync(join(opencodeDir, 'auth', 'openai.json'), '{}');
		writeFileSync(join(opencodeDir, 'openai-codex-auth-multi-config.json'), '{}');
		writeFileSync(join(opencodeDir, 'logs', 'codex-plugin-multi', 'log.txt'), 'log');
		...
		expect(existsSync(join(opencodeDir, 'openai-codex-auth-multi-config.json'))).toBe(false);
		expect(existsSync(join(opencodeDir, 'logs', 'codex-plugin-multi'))).toBe(false);
