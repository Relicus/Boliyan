-- Migration: Increase Bid Limit to 5 (Initial + 4 Updates)
-- Date: 2026-01-25
...
DECLARE
  ...
  min_price DECIMAL;
  max_price DECIMAL;
  max_updates CONSTANT INTEGER := 4; -- 0 (initial) + 4 updates = 5 total attempts
BEGIN
  ...
  IF (TG_OP = 'UPDATE') THEN
    ...
    IF NEW.amount != OLD.amount THEN
      IF OLD.update_count >= max_updates THEN
        RAISE EXCEPTION 'Out of Bids: You have reached the maximum of 5 attempts for this item.';
      END IF;
